...
/* ---------- master Build Plan (the unified journey's single deliverable) ---------- */
...
function PlanPdf({ data }: { data: PlanPdfData }) {
  const firstName = data.name.split(" ")[0] || data.name
  return (
    <Document author="South Eastern General Contractors" title={`SEGC Master Build Plan — ${data.name}`}>
      <Page size="LETTER" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.brand}>South Eastern General Contractors</Text>
          <Text style={styles.preHeader}>■ YOUR MASTER BUILD PLAN ■</Text>
          <Text style={styles.h1}>{firstName.toUpperCase()}, THIS IS YOUR BUILD.</Text>
        </View>
        <View style={styles.body}>
          <Text style={styles.bigNumberLabel}>Estimated planning range</Text>
          <Text style={styles.bigNumber}>{data.headline}</Text>
          ...
          {data.sections.map((section) => (
            <View key={section.title} style={styles.section} wrap={false}>
              <View style={styles.tableHeader}>
                <Text style={styles.cellLabel}>{section.title}</Text>
              </View>
              {section.rows.map(([label, value], index) => (
                ...
                >
                  <Text style={styles.cellLabel}>{label}</Text>
                  <Text style={{ ...styles.cellValue, width: 240 }}>{value}</Text>
                </View>
              ))}
            </View>
          ))}

          <View style={styles.ctaBand} wrap={false}>
            <Text style={styles.preHeader}>■ WHAT HAPPENS NEXT ■</Text>
            <Text style={styles.ctaHeading}>Turn this into a real plan.</Text>
            <Text style={styles.ctaText}>{data.note}</Text>
            <View style={{ flexDirection: "row" }}>
        ...
        <Text style={styles.footer} fixed>
          Your Master Build Plan, generated by The SEGC Build Planner on {data.generatedAt}. All
          figures are directional planning guidance, not a quote, bid, lending decision, or offer to
          build. South Eastern General Contractors · Fayetteville, NC · 20+ Years · SBA 8(a) &
          HUBZone Certified.
        </Text>
      </Page>
    </Document>
